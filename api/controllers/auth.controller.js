...
export const signin = async (req, res, next) => {
    ...
    try {
        ...
        const { password: pwd, ...restInfo } = validUser._doc

        res.cookie('access_token', token, { httpOnly: true }).status(200).json(restInfo) // need to add maxAge or expire
...
export const google = async (req, res, next) => {
    ...
    try {
        ...
        if (user) {
            ...
            const { password: pwd, ...restInfo } = user._doc
            ...
            const { password: pwd, ...restInfo } = newUser._doc
            res.cookie('access_token', token, { httpOnly: true }).status(200).json(restInfo) // need to add maxAge or expire
        }
    } catch (err) {
        next(err)
